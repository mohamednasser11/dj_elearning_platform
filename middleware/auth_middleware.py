...
class AuthMiddleware:
    ...
    def auth_check(self, request):
        if request.path not in VALID_URLS and (
            "Authorization" not in request.headers or not self.is_token_valid(request)
        ):
    ...
    def unauthorized_response(self, message):
        return JsonResponse(
            {"error": str(message)}, status=status.HTTP_401_UNAUTHORIZED
        )
